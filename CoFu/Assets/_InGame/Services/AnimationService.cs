...
{
    public Tween CreateTween(GameObject obj, ItemAnimation anim)
    {
    ...
    // === Individual Tween Methods ===

    public Tween Fade(GameObject obj, float to, float duration)
    {
    ...
    public Tween Scale(GameObject obj, Vector3 from, Vector3 to, float duration)
    {
        obj.transform.localScale = from;
        return obj.transform.DOScale(to, duration);
    }

    public Tween Slide(GameObject obj, Vector3 from, Vector3 to, float duration)
    {
        obj.transform.localPosition = from;
        return obj.transform.DOLocalMove(to, duration);
    }

    public Tween Rotate(GameObject obj, Vector3 from, Vector3 to, float duration)
    {
        obj.transform.rotation = Quaternion.Euler(from);
        return obj.transform.DORotate(to, duration);
    }

    public Tween Punch(GameObject obj, float strength, float duration, int vibrato)
    {
        return obj.transform.DOPunchScale(Vector3.one * strength, duration, vibrato);
    }

    public Tween Bounce(GameObject obj, Vector3 from, Vector3 to, float duration)
    {
        obj.transform.localScale = from;
        return obj.transform.DOScale(to, duration).SetEase(Ease.OutBounce);
    }

    public Tween FadeScale(GameObject obj, float alpha, Vector3 scale, float duration)
    {
